function data = Partition( obj, folds, fold, seed )
...
% James Lloyd, June 2012
  ...
  if ~isempty(obj.file_name) 
    ...
    fold_file_name = [ContainingFolder(end:-1:1) 'DataFolds/' FileName '_' ...
                      num2str(fold, '%02d') 'of' num2str(folds, '%02d') '.mat'];
  else
    fold_file_name = '';
  end
  ...
  elseif folds == 1
    ...
    for i = 1:length(obj.train_X_v)
      % Create partition
      n_observed = length(obj.train_X_v{i});
      perm = randperm(n_observed)';
      boundaries = floor(linspace(1, n_observed, folds+1));
      if fold == 1
