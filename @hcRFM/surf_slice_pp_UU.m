function prop_success = surf_slice_pp_UU( obj, array_index, slice_width, step_out, max_attempts)
%surf_slice_pp_UU Joint slice sampling of pseudo points and targets
...
for mm = randperm(M)
  %Set the slice level
  % This conditional log likelihood is still ok since W_UU updated in
  % calculation
  ...
  full_conditional_surf = obj.K_pp_pp_UU{i}(mm,not_mm) * (obj.K_pp_pp_UU{i}(not_mm, not_mm) ...
                                                       \ obj.T_UU{i}(not_mm));
  surf_height = obj.T_UU{i}(mm) - full_conditional_surf;
  ...
  if step_out
    while attempts < max_attempts
      obj.pp_UU{i}(mm, :) = pp_l;
      % Surf
      obj.UpdateKernelMatrices_pp_UU (mm, i); % This is slightly inefficient
      full_conditional_surf = obj.K_pp_pp_UU{i}(mm,not_mm) * (obj.K_pp_pp_UU{i}(not_mm, not_mm) ...
                                                           \ obj.T_UU{i}(not_mm));
      obj.T_UU{i}(mm) = full_conditional_surf + surf_height;
      test_p = obj.Cond_llh_pp_UU_No_Update(mm, i);
      if test_p > log_Pstar
  ...
  while attempts < max_attempts
    ...
    full_conditional_surf = obj.K_pp_pp_UU{i}(mm,not_mm) * (obj.K_pp_pp_UU{i}(not_mm, not_mm) ...
                                                         \ obj.T_UU{i}(not_mm));
    obj.T_UU{i}(mm) = full_conditional_surf + surf_height;
    log_p_prime = obj.Cond_llh_pp_UU_No_Update(mm, i);
